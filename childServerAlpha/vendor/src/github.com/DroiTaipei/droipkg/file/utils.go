...
func GetCdnURL(fqdn, appid, fid, filename string) string {
	return fmt.Sprintf("http://%s/droi/%s/%s/%s",
		fqdn,
		appid,
		fid,
		url.QueryEscape(GetFileBasename(fid, filename)))
}
...
// IsWangsuURL - if the url hosted by Wangsu
func IsWangsuURL(url string) bool {
	return strings.Contains(url, VendorWangsuDomain)
}
